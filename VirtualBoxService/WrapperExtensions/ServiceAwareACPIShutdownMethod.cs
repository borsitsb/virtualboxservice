...
namespace VirtualBoxService.WrapperExtensions
{
	...
	public class ServiceAwareACPIShutdownMethod : ACPIShutdownMethod
	{
        private IServiceInfoProvider _serviceInfoProvider;

        public override int ACPIShutdownTimeoutMilliSeconds {
            get {
                return _serviceInfoProvider.getACPIShutdownTimeoutMilliSeconds();
            }
            set {
                throw new InvalidOperationException("Timeout can't be set, because it is derived from VM-Description"); //violates LSP :-P
            }
        }
